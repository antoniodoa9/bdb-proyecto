...
\begin{description}
    ...
    \item[Python 3.12.] Lenguaje de scripting para la automatización de la generación de datos y la medición estadística de tiempos de consulta. Los scripts principales son \texttt{generador\_datos.py} (población de la base de datos) y \texttt{medidor\_tiempos.py} (benchmarking).
    ...
    \item[eXist-db y eXide.] Base de datos nativa XML y su editor web integrado. Se usan en la Parte~II para alojar los documentos clínicos transformados y ejecutar sobre ellos las consultas XQuery (FLWOR).

    \item[MongoDB y Compass.] Base de datos documental NoSQL y su interfaz gráfica oficial. Soportan la Parte~III: almacenan las guías clínicas y los expedientes en formato JSON, ejecutan los \textit{aggregation pipelines} y permiten analizar los planes de ejecución con \textit{Explain Plan}.

    \item[Tkinter y PyMongo.] Librerías de Python para, respectivamente, construir la interfaz gráfica de escritorio del sistema de soporte a la decisión y comunicarse con MongoDB desde ella.

    \item[Docker.] Plataforma de contenedores. Tanto el servidor MySQL como la base de datos XML eXist-db se despliegan en contenedores Docker, lo que garantiza un entorno reproducible, aislado e idéntico para todo el equipo, sin instalaciones permanentes sobre el sistema operativo.
\end{description}
...
La \textbf{Parte~I} (Sección~2) constituye el núcleo relacional del trabajo. Describe el diseño e implementación de la fase con MySQL: el modelo de datos, la arquitectura multi-motor, la política de seguridad, el proceso de generación de datos sintéticos y la batería completa de benchmarks de rendimiento temporal y espacial. Cada decisión de diseño se acompaña de la evidencia empírica que la respalda.

La \textbf{Parte~II} (Sección~3) da el salto a las bases de datos semiestructuradas. Explica cómo exportamos los datos relacionales a XML, cómo los reestructuramos con Python para darles una jerarquía clínica con sentido y cómo, una vez cargados en eXist-db, los interrogamos con XQuery para construir tres herramientas de apoyo a la decisión clínica.

La \textbf{Parte~III} (Sección~4) presenta la arquitectura híbrida con MongoDB. Justifica la convivencia de lo relacional y lo documental, el enfoque de privacidad desde el diseño en la carga masiva de datos, el modelado de las guías clínicas en JSON, el análisis de rendimiento con índices y, por último, la interfaz gráfica que integra todo el sistema.

Las \textbf{Conclusiones} (Secciones~5 y~6) recogen los resultados más relevantes del trabajo y los ponen en perspectiva, discuten el grado de consecución de los objetivos y proponen líneas de investigación futura.
