...
<<<<<<< HEAD
Sobre esta base relacional, el trabajo se extiende a otros dos paradigmas de almacenamiento. En la fase semiestructurada, los datos se exportan a XML, se reestructuran con Python para dotarlos de una jerarquía clínica con sentido y se cargan en \textbf{eXist-db}, donde tres consultas \textbf{XQuery} (FLWOR) actúan como sistema de soporte a la decisión: triangulan el riesgo de los pacientes de urgencias con sus antecedentes, detectan pacientes crónicos y generan alertas de hipoxia. En la fase NoSQL se construye una \textbf{arquitectura híbrida con MongoDB} que aloja las guías clínicas como documentos BSON y las aplica a 50.000 expedientes mediante \textit{aggregation pipelines} (\texttt{\$lookup}, \texttt{\$bucket}, \texttt{\$switch}). El análisis del plan de ejecución (\texttt{.explain()}) confirmó que la indexación sustituye el escaneo completo de la colección (\textbf{COLLSCAN} sobre 50.000 documentos) por un recorrido de índice (\textbf{IXSCAN}) que examina solo los 12.040 documentos relevantes. Todo el sistema se integra en una interfaz gráfica en Python que delega el procesamiento masivo en el motor de MongoDB, manteniendo el principio de privacidad desde el diseño: el DNI se transforma en un hash SHA-256 en la inserción inicial y nunca abandona la capa transaccional. Los servidores de datos (MySQL y eXist-db) se despliegan en contenedores \textbf{Docker} para garantizar un entorno reproducible y aislado.

\bigskip
\noindent\bfseries{\large{Palabras clave:}} MySQL, InnoDB, ARCHIVE, MEMORY, RGPD, seudonimización, historiales médicos longitudinales, benchmarking de consultas, indexación, XML, XQuery, eXist-db, MongoDB, BSON, aggregation pipeline, soporte a la decisión clínica, Docker, Python.
=======
