...
The project demonstrates that the tension between GDPR compliance and operational performance is a false dilemma. Compliance by design, through structural data separation, and performance optimisation, through engine selection and indexing, are not competing goals but complementary architectural decisions that reinforce each other. The additional complexity of a multi-engine schema is fully justified by the quantified gains in both dimensions.
...
\paragraph{Extension to non-relational paradigms (Part~II).}
The structured schema of ChronoHealthDB is well suited for tabular clinical data, but modern healthcare systems generate information that does not fit naturally into rigid relational structures. Two non-relational paradigms are particularly relevant:

\begin{itemize}
    \item \textbf{Graph databases (Neo4j / Apache AGE on PostgreSQL)}: clinical networks are inherently relational in the graph-theoretic sense. Co-morbidity networks, drug interaction graphs, care pathway analyses and epidemic contact tracing are examples of queries that express naturally as graph traversals but are expensive as multi-join SQL operations. A natural extension would be to project the \texttt{evento\_clinico} table into a property graph and evaluate the performance of graph algorithms (shortest path, community detection) on anonymised patient data.

    \item \textbf{Document databases (MongoDB)}: unstructured clinical notes, imaging reports and discharge summaries are currently truncated or omitted from the relational model due to the schema's fixed structure. A document store as a complement to the relational layer would accommodate free-text clinical content with full-text search capabilities, while the relational layer retains structured event data.
\end{itemize}
