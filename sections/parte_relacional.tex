...
En lugar de instalar MySQL Server directamente sobre el sistema operativo, optamos por levantarlo dentro de un contenedor \textbf{Docker}~\cite{docker}. La ventaja es doble: por un lado, conseguimos un entorno reproducible e idéntico para los tres integrantes del grupo, sin depender de la versión que cada uno tuviera instalada ni \enquote{ensuciar} la máquina con servicios permanentes; por otro, podemos arrancar, parar o borrar el servidor entero con un solo comando. Bastó con descargar la imagen oficial de MySQL de Docker Hub y arrancar el contenedor mapeando el puerto~3306 para poder conectarnos después desde MySQL Workbench (Listado~\ref{lst:docker_mysql}).
...
Antes de nada conviene justificar por qué trabajamos con datos \textit{sintéticos} y no con datos reales de pacientes. La razón es sencilla: los historiales clínicos son probablemente la categoría de información personal más sensible que existe, y precisamente por eso el RGPD~\cite{rgpd2016} los blinda como datos especialmente protegidos. Conseguir un volumen real de decenas de miles de historiales habría exigido la aprobación de un comité de ética, el consentimiento de los pacientes y unas garantías de custodia que quedan completamente fuera del alcance de un trabajo académico. Generar los datos de forma sintética nos permite disponer de un volumen grande y realista para los benchmarks sin manejar ni un solo dato real de una persona, lo que además encaja a la perfección con la filosofía de privacidad por diseño que defiende todo el proyecto.
